...
- `docs/plans/*.md` — Plan files created by `/gc:plan`. These are living documents that track implementation progress (checkboxes are checked off by `/gc:work`).
...
**Tagging:** Always add `code-review` tag, plus: `security`, `performance`, `architecture`, `gdscript`, `quality`, etc.
...
- gc-code-simplicity-reviewer
- gc-pattern-recognition-specialist
...
### 6. Quality Validation (Optional)

After presenting the Summary Report, offer to run linting:
