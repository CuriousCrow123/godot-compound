...
- Document relevant file paths from repo research (e.g., `scripts/player/player_controller.gd:42`)
...
```gdscript
# scripts/player/player_controller.gd:42
...
**Note:** If running `/gc:plan` with ultrathink enabled, automatically run `/deepen-plan` after plan creation for maximum depth and grounding.

Loop back to options after Simplify or Other changes until user selects `/gc:work` or `/technical_review`.
...
5. **After creation:**
   ...
   - Ask if they want to proceed to `/gc:work` or `/technical_review`
